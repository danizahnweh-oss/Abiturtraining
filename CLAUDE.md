...
## Projekt-Struktur
- **Statische Seiten** (HTML + Vanilla JS): Root-Verzeichnis → deployed via **Hetzner** auf myabiflow.de
- **Kolloquiumstrainer** (React SPA): `abitur-kolloquium-trainer/` → Vite + React 19 + TypeScript + Tailwind 4
- **Backend API**: `src/index.js` → läuft auf Hetzner
- **Gemini Proxy**: `abitur-kolloquium-trainer/worker/src/index.ts` → läuft auf Hetzner

## Build & Deploy
- **Statische Seiten + Kolloquiumstrainer:** `bash build-pages.sh` → Deploy auf Hetzner
- Backend / Gemini Proxy: Deploy auf Hetzner
...
- Reihenfolge bei statischen Seiten / Kolloquiumstrainer: `bash build-pages.sh` → Deploy auf Hetzner → git commit & push
- Reihenfolge bei Backend-Änderungen: Deploy auf Hetzner → git commit & push
